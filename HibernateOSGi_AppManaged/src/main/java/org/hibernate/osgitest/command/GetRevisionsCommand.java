...
public class GetRevisionsCommand implements Action {
    ...
    public Object execute(CommandSession session) throws Exception {
    	Map<Number, DataPoint> revisions = dpService.getRevisions(Long.valueOf( id ));
        for (Number revisionNum : revisions.keySet()) {
        	DataPoint dp = revisions.get( revisionNum );
            System.out.println(revisionNum + ": " + dp.getId() + ", " + dp.getName());
        }
        return null;
    }
